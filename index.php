...
foreach ($ms['lines'] as $item) {
	...
	// echo "INSERT INTO BusLineTable (Air,Auto,BasicPrice,CommunicationTicket,Company,DataSource,Description,StartTime,EndTime,FrontName,FrontSpell,KeyName,Length,Name,Status,TerminalName,TerminalSpell,TotalPrice,Type,IsDoubleDeck,IsExpressWay,IsIcCard,Coordinates) VALUES ({$item['air']},{$item['auto']},{$item['basic_price']},{$item['commutation_ticket']},'{$item['company']}',1,'{$item['description']}','{$item['start_time']}','{$item['end_time']}','{$item['front_name']}','{$item['front_spell']}','{$item['key_name']}',{$item['length']},'{$item['name']}',{$item['status']},'{$item['terminal_name']}','{$item['terminal_spell']}',{$item['total_price']},{$item['type']},{$item['double_deck']},{$item['express_way']},{$item['ic_card']},'{$item['coordinates']}')";
	$db->exec("INSERT INTO BusLineTable (Air,Auto,BasicPrice,CommunicationTicket,Company,DataSource,Description,StartTime,EndTime,FrontName,FrontSpell,KeyName,Length,Name,Status,TerminalName,TerminalSpell,TotalPrice,Type,IsDoubleDeck,IsExpressWay,IsIcCard,Coordinates) VALUES ({$item['air']},{$item['auto']},{$item['basic_price']},{$item['commutation_ticket']},'{$item['company']}',1,'{$item['description']}','{$item['start_time']}','{$item['end_time']}','{$item['front_name']}','{$item['front_spell']}','{$item['key_name']}',{$item['length']},'{$item['name']}',{$item['status']},'{$item['terminal_name']}','{$item['terminal_spell']}',{$item['total_price']},{$item['type']},{$item['double_deck']},{$item['express_way']},{$item['ic_card']},'{$item['coordinates']}')");
